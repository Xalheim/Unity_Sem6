...
public class UnlockRocketLauncher : MonoBehaviour
{
    [SerializeField]
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (!triggered && other.CompareTag("Player"))
        {
            ...
            if (!playerInteraction.VerifyWeaponPossession(3))
            {
                progressManager.UnlockRocketLauncher();
                playerInteraction.SwitchToWeapon(3);
            }
            triggered = true;
            arena.ActivateArena();
            Destroy(parent);
